...
export function Particles({ mousePosRef }: { mousePosRef: React.MutableRefObject<THREE.Vector3 | null> }) {
  ...
  const spawnParticle = (pos: THREE.Vector3, colorHex: string, isAurora = false) => {
    ...
    if (isAurora) {
      // Aurora particles move upwards and have less horizontal spread
      p.velocity.set(
        (Math.random() - 0.5) * 0.5,
        Math.random() * 3.0 + 1.0,
        (Math.random() - 0.5) * 0.5
      );
      p.life = PARTICLE_LIFETIME * 1.5;
    } else {
      p.velocity.set(
        (Math.random() - 0.5) * 2.0,
        (Math.random() - 0.5) * 2.0,
  ...
  useFrame((state, delta) => {
    ...
    if (mousePosRef.current && myColor) {
      ...
      for (let i = 0; i < 80; i++) {
        // 20% chance for aurora streak
        spawnParticle(mousePosRef.current, myColor, Math.random() < 0.2);
      }
    }
    ...
    Object.values(players).forEach(player => {
      if (player.position && player.color) {
        ...
        for (let i = 0; i < 40; i++) {
          spawnParticle(pPos, player.color, Math.random() < 0.1);
        }
      }
    });
